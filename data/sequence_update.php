...
foreach ($lines as $i => $ln)
{
	...
	if (preg_match($match, $ln))
    {
		...
		if (file_exists("../pdbs/$subdir/$rcpid.rotated.pdb"))
			rename("../pdbs/$subdir/$rcpid.rotated.pdb", "../pdbs/$subdir/$rcpid.upright.pdb");

		foreach (["upright", "active", "inactive", "apo", "bound"] as $suffix)
        {
			$pdbname = "../pdbs/$subdir/$rcpid.$suffix.pdb";
			...
			foreach ($pdbdat as $pdbln)
			{
				if (substr($pdbln, 0, 6) == "REMARK")
				{
					...
					if ($remno < 600) $remarks[] = $pdbln;
				}
				else $not_remarks[] = $pdbln;
			}

			$not_remarks = implode("\n", $not_remarks);
			$not_remarks = str_replace("\n\nTER\nEND", "\nTER\nEND", $not_remarks);
