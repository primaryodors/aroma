...
$cinnanoms =										// sienna is a cinnanom of synomyn
[
	"o-" => "ortho-",
	"m-" => "meta-",
	"p-" => "para-",
...
foreach ($odors as $oid => $odor)
{
	...
	{
		if (!is_array($aroma)) die("Data error: ".print_r($odor, true));
	...
	if (isset($o["isomers"]))
	{
		...
		foreach ($o["isomers"] as $iso => $ismiles)
		{
			...
			if (!file_exists($isofname) || filesize($isofname) < 20)
			{
				...
				if ($first)
				{
					if (file_exists("sdf/$fullname.sdf")) unlink("sdf/$fullname.sdf");
