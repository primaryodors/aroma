...
foreach ($orlist as $orid => $strength)
{
    ...
    if (preg_match("/^[01]+$/", $orid))
    {
        if (isset($treenodes[$orid]))
        {
            echo "<tr><th width=\"5%\" style=\"white-space: nowrap;\"><a href=\"receptors.php?tree=1&bt=$orid\">{$treenodes[$orid]}</a></th>
                <td><div style=\"display: inline-block; background-color: #ffc; width: $width%;\">&nbsp;</div></td></tr>\n";
        }
        else echo "\n<!-- unnamed node $orid --->";
        continue;
    }
    echo "<tr><th width=\"5%\" style=\"white-space: nowrap;\"><a href=\"receptor.php?r=$orid\">$orid</a></th>
        <td><div style=\"display: inline-block; background-color: #ffc; width: $width%;\">&nbsp;</div></td></tr>\n";
}
