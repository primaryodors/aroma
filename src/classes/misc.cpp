...
{
    return (-R * temperature)*log(K);
}

double K(double DeltaG)
{
    return exp(DeltaG / (-R * temperature));
}

double equilibrium(double kJmol1, double kJmol2)
{
    double DeltaG = kJmol2 - kJmol1;
    double lK = K(DeltaG);
    return lK/(lK+1);
}
...
bool end_program = false, end_iterations = false;
