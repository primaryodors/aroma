...
void raise_error(std::string message)
{
    ...
    if (fp)
    {
        ...
        char time[256];
        strftime(time, 256, "%F %T", gmtime(&now));
        //

        fprintf(fp, "%s %s: %d Error: %s\n", time, script_fname.c_str(), program_counter+1, message.c_str());
        fclose(fp);
    }
...
int main(int argc, char** argv)
{
    ...
    {
        /*cout << "Usage:" << endl << "phew protein.pdb script_filename" << endl;
        cout << "phew script_filename protein.pdb" << endl;*/
    ...
    while (program_counter < script_lines.size())
    {
        ...
        _interpret_command:
            ...
            else if (!strcmp(words[0], "BRIDGE"))
            {
                ...
                working->bridge(r1, r2);

            }	// BRIDGE

            else if (!strcmp(words[0], "XBRIDGE"))
            {
                int r1, r2, strandidx, iters=50;

                l = 1;
                if (!words[l]) raise_error("Insufficient parameters given for XBRIDGE.");
                r1 = interpret_single_int(words[l++]);

                if (!words[l]) raise_error("Insufficient parameters given for XBRIDGE.");
                if (strlen(words[l]) != 1) raise_error("Invalid strand ID given for XBRIDGE.");
                strandidx = words[l][0] - 'A';
                if (!strands[strandidx]) raise_error("Empty strand ID given for XBRIDGE.");
                l++;

                if (!words[l]) raise_error("Insufficient parameters given for XBRIDGE.");
                r2 = interpret_single_int(words[l++]);

                if (words[l]) raise_error("Too many parameters given for XBRIDGE.");

                Molecule* mols[4];
                mols[0] = (Molecule*)working->get_residue(r1);
                if (!mols[0]) raise_error("Local resno not found in working protein.");
                mols[0]->movability = MOV_FLEXONLY;
                mols[1] = (Molecule*)strands[strandidx]->get_residue(r2);
                if (!mols[1]) raise_error("Remote resno not found in other protein.");
                mols[1]->movability = MOV_FLEXONLY;
                mols[2] = mols[3] = nullptr;
                Molecule::conform_molecules(mols);
            }	// XBRIDGE

            else if (!strcmp(words[0], "BW50"))
            {
                l = 1;
                if (!words[l]) raise_error("Insufficient parameters given for BW50.");
                int bwhx = interpret_single_int(words[l++]);
                if (!words[l]) raise_error("Insufficient parameters given for BW50.");
                int res50 = interpret_single_int(words[l++]);
                if (words[l]) raise_error("Too many parameters given for BW50.");
                working->set_bw50(bwhx, res50);
            }	// BW50
            ...
            else if (!strcmp(words[0], "BWCENTER"))
            {
                ...
                for (l=1; l<=7; l++)
                {
                    ...
                    if (resno < 1) raise_error((std::string)"Strand does not have a " + std::to_string(l) + (std::string)".50 residue.");
                    AminoAcid* aa = working->get_residue(resno);
                    if (!aa) raise_error((std::string)"Residue " + std::to_string(resno) + (std::string)" not found in strand.");
                    bw50_CA[l] = aa->get_CA_location();
                    bw50_CA[l].weight = 1;
                }
            ...
            else if (!strcmp(words[0], "DBGBWCEN"))
            {
                ...
                for (l=1; l<=7; l++)
                {
                    ...
                    cout << "Residue " << resno << " CA is located at " << a->loc
            ...
            else if (!strcmp(words[0], "BWMOTIF"))
            {
                ...
                // else cout << "Motif " << words[2] << " found at position " << k << endl;
                // cout << l << ".50 = " << k << endl;
            ...
            else if (!strcmp(words[0], "MOC"))
            {
                ...
                atom1 = aa1->get_nearest_atom(atom2->loc);
                atom2 = aa2->get_nearest_atom(atom1->loc);
                ...
                {
                    if (!optimize.r) optimize = atom2->loc.subtract(atom1->loc);
            ...
            else if (!strcmp(words[0], "DELATOM"))
            {
                if (!words[1]) raise_error("Insufficient parameters given for DELATOM.");
                int resno = interpret_single_int(words[1]);
                if (!words[2]) raise_error("Insufficient parameters given for DELATOM.");
                char* aname = interpret_single_string(words[2]);
                if (words[3]) raise_error("Too many parameters given for DELATOM.");

                AminoAcid* aa = working->get_residue(resno);
                if (!aa) raise_error((std::string)"Residue " + std::to_string(resno) + (std::string)" not found in protein.");
                Atom* a = aa->get_atom(aname);
                if (a) aa->delete_atom(a);
            } // DELATOM
            ...
            else if (!strcmp(words[0], "DOWNLOAD"))
            {
                ...
                if (!skip_if_exists || !file_exists(destfn))
                {
                    ...
                    while (!feof(fp))
                    {
                        char* got = fgets(buffer1, 1022, fp);
                        if (!got) break;
                    ...
                    else if (retfmt.substr(0, 5) == "JSON:")
                    {
                        ...
                        n = key.length();
                        if (!download_file(url, "tmp/.protdl")) raise_error("JSON retrieval failed.");
                        FILE* fp = fopen("tmp/.protdl", "rb");
            ...
            else if (!strcmp(words[0], "HYDRO") || !strcmp(words[0], "DEHYD"))
            {
                bool dehyd = !strcmp(words[0], "DEHYD");
                int resno, endres = working->get_end_resno();
                cout << (dehyd ? "Dehydrogenating..." : "Hydrogenating...");
                for (resno=1; resno<=endres; resno++)
                {
                    AminoAcid* res = working->get_residue(resno);
                    if (res)
                    {
                        if (dehyd) res->dehydrogenate();
                        else
                        {
                            res->hydrogenate();
                        }
            ...
            else if (!strcmp(words[0], "INTER"))
            {
                l = 1;

                if (!words[l]) raise_error("Insufficient parameters for INTER.");
                char* str1 = interpret_single_string(words[l]);
                if (strlen(str1) > 1 || str1[0] < 'A' || str1[0] > 'Z') raise_error((std::string)"Bad strand ID: " + (std::string)str1);
                l++;
                
                if (!words[l]) raise_error("Insufficient parameters for INTER.");
                char* str2 = interpret_single_string(words[l]);
                if (strlen(str2) > 1 || str2[0] < 'A' || str2[0] > 'Z') raise_error((std::string)"Bad strand ID: " + (std::string)str2);
                l++;
                
                if (!words[l]) raise_error("Insufficient parameters for INTER.");
                char* varname = words[l++];

                if (words[l]) raise_error("Too many parameters for INTER.");

                int strand1 = str1[0] - 'A';
                int strand2 = str2[0] - 'A';

                if (!strands[strand1]) raise_error((std::string)"Strand " + (std::string)str1 + (std::string)" not loaded.");
                if (!strands[strand2]) raise_error((std::string)"Strand " + (std::string)str2 + (std::string)" not loaded.");

                int seqlen1 = strands[strand1]->get_end_resno(), seqlen2 = strands[strand2]->get_end_resno();
                // cout << "Sequence lengths: " << seqlen1 << ", " << seqlen2 << endl;
                AminoAcid* interface[1024];
                int ninter = 0;
                bool already[seqlen2+4];
                for (j=1; j<=seqlen2; j++) already[j] = false;
                for (l=1; l<=seqlen1; l++)
                {
                    AminoAcid* aa1 = strands[strand1]->get_residue(l);
                    if (!aa1) continue;
                    bool addedaa1 = false;
                    for (j=1; j<=seqlen2; j++)
                    {
                        AminoAcid* aa2 = strands[strand2]->get_residue(j);
                        if (!aa2) continue;
                        ...
                        {
                            if (!addedaa1) interface[ninter++] = aa1;
                            addedaa1 = true;
                            if (!already[j]) interface[ninter++] = aa2;
                            already[j] = true;
                        }
                    }
                }
                interface[ninter] = nullptr;
                // cout << ninter << " interface residues." << endl;

                Star s;
                float energy = Molecule::total_intermol_binding(reinterpret_cast<Molecule**>(interface)).summed();
                s.f = energy;
                set_variable(varname, s);
            }	// INTER
            ...
            else if (!strcmp(words[0], "MCOORD"))
            {
                ...
                std::vector<string> cratoms;	// coordinating residue atoms.
            ...
            else if (!strcmp(words[0], "MEASURE"))
            {
                ...
                if (!a1)
                {
                    AminoAcid* aa = working->get_residue(res1);
                    raise_error(std::to_string(res1) + (std::string)":" + atom1 + (std::string)" not found in protein."
                        + (std::string)"\nResidue number " + std::to_string(res1)
                        + (aa ? ((std::string)" is of type " + (std::string)aa->get_aa_definition()->_3let + (std::string)".") : (std::string)" is missing.")
                        );
                }
                ...
                if (!a2)
                {
                    AminoAcid* aa = working->get_residue(res2);
                    raise_error(std::to_string(res2) + (std::string)":" + atom2 + (std::string)" not found in protein."
                        + (std::string)"\nResidue number " + std::to_string(res2)
                        + (aa ? ((std::string)" is of type " + (std::string)aa->get_aa_definition()->_3let + (std::string)".") : (std::string)" is missing.")
                        );
                }
            ...
            {
                l = 1;
                if (!words[l]) raise_error("Insufficient parameters given for PROT.");
				if (words[l+1]) raise_error("Too many parameters given for PROT.");
                int resno = interpret_single_int(words[l]);
                AminoAcid* aa = working->get_residue(resno);
                if (!aa) raise_error((std::string)"Residue " + std::to_string(resno) + (std::string)" not found in protein.");
                bool result = aa->protonate();
                if (!result) cerr << "Warning: failed to protonate " << aa->get_3letter() << resno << "." << endl;
            }	// PROT

            else if (!strcmp(words[0], "DEPROT"))
            {
                l = 1;
                if (!words[l]) raise_error("Insufficient parameters given for DEPROT.");
				if (words[l+1]) raise_error("Too many parameters given for DEPROT.");
                int resno = interpret_single_int(words[l]);
                AminoAcid* aa = working->get_residue(resno);
                if (!aa) raise_error((std::string)"Residue " + std::to_string(resno) + (std::string)" not found in protein.");
                bool result = aa->deprotonate();
                if (!result) cerr << "Warning: failed to deprotonate " << aa->get_3letter() << resno << "." << endl;
            }	// DEPROT
            ...
            else if (!strcmp(words[0], "RESAT"))
            {
                l = 1;
				if (!words[l]) raise_error("Insufficient parameters given for RESAT.");
                int resno = interpret_single_int(words[l++]);
                AminoAcid* aa = working->get_residue(resno);
                if (!aa) raise_error((std::string)"Residue " + std::to_string(resno) + (std::string)" not found in protein.");
				if (!words[l]) raise_error("Insufficient parameters given for RESAT.");
				if (words[l+1]) raise_error("Too many parameters given for RESAT.");
                Star svar;
                ...
                switch (words[l][0])
                {
                    case '$':
                    svar.psz = new char[2];
                    svar.psz[0] = aa->get_letter();
                    svar.psz[1] = 0;
                    break;

                    case '@':
                    svar.ppt = new Point(aa->get_CA_location());
                    break;

                    default:
                    raise_error("Unsupported variable type for RESAT.");
                }

                set_variable(words[l], svar);
            }	// RESAT
            ...
            {
                l = 1;
                if (!words[l]) raise_error("Insufficient parameters given for ROTBOND.");
                int resno = interpret_single_int(words[l++]);
                if (!words[l]) raise_error("Insufficient parameters given for ROTBOND.");
                std::string a1name = interpret_single_string(words[l++]);
                if (!words[l]) raise_error("Insufficient parameters given for ROTBOND.");
                std::string a2name = interpret_single_string(words[l++]);
                if (!words[l]) raise_error("Insufficient parameters given for ROTBOND.");
                float theta = interpret_single_float(words[l++]);
                if (words[l]) raise_error("Too many parameters given for ROTBOND.");

                AminoAcid* aa = working->get_residue(resno);
                if (!aa) raise_error((std::string)"Residue " + std::to_string(resno) + (std::string)" not found in protein.");
                Atom* a1 = aa->get_atom(a1name.c_str());
                if (!a1) raise_error((std::string)"Atom " + std::to_string(resno) + (std::string)":" + a1name + (std::string)" not found in protein.");
                Atom* a2 = aa->get_atom(a2name.c_str());
                if (!a2) raise_error((std::string)"Atom " + std::to_string(resno) + (std::string)":" + a2name + (std::string)" not found in protein.");

                Bond* b = a1->get_bond_between(a2);
                if (!b) raise_error((std::string)"Atoms " + std::to_string(resno) + (std::string)":" + a1name + (std::string)" and "
                    + a2name + (std::string)" are not bonded.");
                if (!b->can_rotate) if (!b) raise_error((std::string)"Bond between atoms "
                    + std::to_string(resno) + (std::string)":" + a1name + (std::string)" and "
                    + a2name + (std::string)" cannot rotate.");
                
                b->rotate(theta*fiftyseventh);
            }   // ROTBOND

            else if (!strcmp(words[0], "HISFLIP"))
            {
                l = 1;
                if (!words[l]) raise_error("Insufficient parameters given for HISFLIP.");
                int resno = interpret_single_int(words[l++]);
                if (words[l]) raise_error("Too many parameters given for HISFLIP.");
                AminoAcid* aa = working->get_residue(resno);
                if (!aa) raise_error((std::string)"Residue " + std::to_string(resno) + (std::string)" not found in protein.");
                if (!aa->hisflips) aa->find_his_flips();
                if (!aa->hisflips || !aa->hisflips[0]) raise_error((std::string)"Residue " + std::to_string(resno) + " cannot histidine-flip.");
                aa->do_histidine_flip(aa->hisflips[0]);
            }   // HISFLIP
