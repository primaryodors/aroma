...
PHEW: Protein Hydrogenation and Editing Workspace.

To run a PHEW script, after building AromaDock, please use the following command:

```
bin/phew path/to/script.phew
```
...
PHEW allows holding up to 26 strands in memory at any given time.
...
variable name can occur anywhere in a PHEW variable, there can be more than one nested variable, and variable names can be nested multiple levels
deep, so for example `$array[%i][%array2[%j]]` is legal PHEW syntax as long as `%i`, `%j`, and `%array2[%j]` have all been set.
...
Example:
```
BW50 3 122          # Set residue 122 as the 3.50 residue.
```

Assigns a Ballesteros-Weinstein number to a given residue in the working protein.
...
the same protein, one of which is missing the region definitions, for example if one of the models is included with AROMA and the other is not.
...
ONCE is an optional final parameter that tells PHEW not to request the source URL if the destination file already exists.
...
IF NOT EXISTS "bin/aromadock" DIE "Please compile AromaDock."
```
...
Turns the active protein "upright", so that the extracellular domain is in the +Y direction and the cytoplasmic domain is in the -Y direction.
